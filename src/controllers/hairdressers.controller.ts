...
async function setServiceToUser(req: Request, res: Response) {
    ...
    try {
        const userId = req.params.id;
        const serviceIds: mongoose.Types.ObjectId[] = req.body.services;
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, { $addToSet: { services: { $each: serviceIds } } }, { new: true }).populate('services');
...
async function getHairdressers(req: Request, res: Response) {
    try {
        ...
        const hairdressers = await User.find({ role: 'Hairdresser' }).populate('services');

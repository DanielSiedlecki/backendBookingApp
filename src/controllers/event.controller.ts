import { specialDays, SpecialDaysSchemaDocument } from "../schemats/specialDaysSchema";
...
async function createEvent(req: Request, res: Response) {
    try {
        ...
        res.status(201).json({ message: 'Evente succesfull created', event: newEvent });
    } catch (error) {
        console.error(error);
...
async function getAvailableHours(req: Request, res: Response) {
    try {
        ...
        const specialDay: SpecialDaysSchemaDocument | null = await specialDays.findOne({ date: date });
        ...
        if (specialDay) {
            ...
            if (normalDay) {
                ...
                return res.status(404).json({ message: 'No opening hours information available for this day.' });
            }
        }

        const availableHours: string[] = [];
        let currentTime = new Date(date);
        currentTime.setHours(parseInt(openTime.split(':')[0]), parseInt(openTime.split(':')[1]));

        const closingTime = new Date(date);
        closingTime.setHours(parseInt(closeTime.split(':')[0]), parseInt(closeTime.split(':')[1]));
